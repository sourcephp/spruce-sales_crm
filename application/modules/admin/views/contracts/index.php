<script>
 ...
 function delete_contracts( contract_id )
 {
     
    $.ajax({

        type: "GET",
        url: "<?php echo base_url('admin/contracts/delete' ); ?>/" + contract_id,
        success: function(msg)
        {
			if( msg == 'deleted' )
            {
                $('#contract_id_' + contract_id).fadeOut('normal');
            }
        }
        ...
        <div class="page-content">
            ...
            <div class="breadcrumb-wrapper">
               <?php if (check_staff_permission('contracts_write')){?> 
			  <a href="<?php echo base_url('admin/contracts/add/'); ?>" class="btn btn-primary btn-embossed"> Create New</a> 
			  <?php }?>	
            </div>           
          </div>
                    ...
                    <tbody>
			         ...
			         <?php } else if( $expiration_date <= $next_week) { 
			               ...
			               <?php }?>
	                        ...
	                        <?php if (check_staff_permission('contracts_write')){?> 
	                        <a href="<?php echo base_url('admin/contracts/update/'.$contract->id); ?>" class="edit btn btn-sm btn-default dlt_sm_table"><i class="icon-note"></i></a> 
	                        <?php }?>
	                        
	                        <?php if (check_staff_permission('contracts_delete')){?> 
	                        <a href="javascript:void(0)" class="delete btn btn-sm btn-danger dlt_sm_table" data-toggle="modal" data-target="#modal-basic<?php echo $contract->id; ?>"><i class="glyphicon glyphicon-trash"></i></a>
	                        <?php }?>
	                        </td> 
	                      </tr>
